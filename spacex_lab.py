...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        fig = px.pie(filtered_df, values='class', names='Launch Site', title='Total Successful Launches', hole=0.3)
        return fig
    else:
        filtered_df = spacex_df.loc[spacex_df['Launch Site'] == entered_site]
        colr = []
        s_f = []
        for cls in filtered_df['class']:
            if cls == 0:
                colr.append('red')
                s_f.append('Failure')
            else:
                colr.append('green')
                s_f.append('Success')
        filtered_df['col'] = colr
        filtered_df['outcome'] = s_f
        fig = px.pie(filtered_df, names='outcome', title='Launch success at '+entered_site, hole=0.3,
                     color='col',color_discrete_map={'red':'red','green':'green'})
        return fig
# TASK 4:
